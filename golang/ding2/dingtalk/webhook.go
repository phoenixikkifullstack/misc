...
func (d *DingTalk) sendMessage(msg iDingMsg) error {
    ...
    if resp.StatusCode == http.StatusOK {
        ...
        if err := json.Unmarshal(body, &ret); err != nil {
            return fmt.Errorf("send dingTalk message failed")
        }

        if ret.ErrCode != 0 {
            return fmt.Errorf("send dingTalk message failed, %d", ret.ErrCode)
        }
        return nil
        /*
        // decode json may be better, TODO
        c := strings.Contains(string(body), "errcode\":0")
        if c {
            return nil
        } else {
